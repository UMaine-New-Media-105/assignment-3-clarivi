...
function draw() {
  ...
  if (isSeedAvailable === 1) {
    buildSeed(seedX, fallY);
    fallY = fallY - seedFallSpeed;
    if (fallY <= -225) {
      isSeedAvailable = 0;
      fallY = 0;
      summonCow();
  ...
  if (
    fallY <= -180 &&
    fallY >= -200 &&
    seedX >= (mouseX - 200) / 1.9 - 15 &&
    seedX <= (mouseX - 200) / 1.9 + 15
  ) {
    //If the UFO and the cow collide, 'collect' the cow and spawn a new one. Increment the score
    isSeedAvailable = 0;
    fallY = 0;
    print("Caught!")
    summonCow();
  ...
  if (
    ...
    // If the UFO is above the cow, and the tractor beam is on, the cow will begin to rise faster
    seedFallSpeed = seedFallSpeed * 1.015;
  }
  // ADD CATCHER
...
function buildCatcher(x, y, isMousePressed) {
  ...
  noStroke();
  if (isMousePressed == 1) {
    //Tractor beam turns on
    fill("rgba(44,255,44,0.4)");
    beginShape();
    vertex(-13, 30);
    vertex(-50, 150);
    vertex(50, 150);
  ...
  if (isMousePressed == 1) {
    //Create dots on the UFO. If mouse is pressed, the 'windows' will light up
    fill("yellow");
  } else {
    fill("black");
  }
...
function buildSeed(x, y) {
  //builds object to be used as seed. in this case, a cow!
  push();
  stroke("black");
  ...
  ellipse(203, 340, 0.5); // it's eye
  pop();
}

function summonCow() {
  if (isSeedAvailable === 0) {
    isSeedAvailable = 1;
    seedX = random(-100, 95);
    seedFallSpeed = random(0.75, 1.25);
  }
}
